...
using FIS.USA.POC.EventTypeA.Plugin;
using FIS.USA.POC.EventTypeA.Shared;
using FIS.USESA.POC.Plugins.Interfaces.Entities.Config;
using FIS.USESA.POC.Plugins.Interfaces.Entities.Kafka;

namespace FIS.USA.POC.EventTypeA.Consumer
{
    /// <summary>
    ...
    class Program
    {
        private const string CONSUMER_GROUP_NAME = @"EVENT_TYPE_A_CONSUMER_GROUP";
        ...
        {
            string topicName = EventTypeAPublisher.TOPIC_NAME;

            using var schemaRegistry = new CachedSchemaRegistryClient(_schemaRegistryConfig);
            using var consumer = new ConsumerBuilder<EventKey, EventTypeABody>(_consumerConfig)
                                        .SetKeyDeserializer(new AvroDeserializer<EventKey>(schemaRegistry).AsSyncOverAsync())
                                        .SetValueDeserializer(new AvroDeserializer<EventTypeABody>(schemaRegistry).AsSyncOverAsync())
