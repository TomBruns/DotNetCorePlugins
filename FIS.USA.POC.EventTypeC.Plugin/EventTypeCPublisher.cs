...
using FIS.USA.POC.EventTypeC.Shared;
...
namespace FIS.USA.POC.EventTypeC.Plugin
{
    /// <summary>
    /// This is a plug-in class and knows how to publish EventTypeC events
    /// </summary>
    ...
    public class EventTypeCPublisher : IEventPublisher
    {
        ...
        public async Task<PublishMsgResultsBE> PublishEvent(int eventId, string message)
        {
            ...
            var eventBody = new EventTypeCBody()
            {
                id = eventId,
                name = message,
            ...
            // ====================================
            var eventPubResult = await KafkaUtilities.PublishMsgToTopic<EventKey, EventTypeCBody>(TOPIC_NAME, eventKey, eventBody, _kafkaConfig);

            if (!eventPubResult.IsSuccess)
            {
                errTitle = $"Error in {nameof(EventTypeCPublisher)}";
