...
namespace FIS.USA.POC.EventTypeA.Plugin
{
    /// <summary>
    ...
    {
        public static readonly string TOPIC_NAME = @"EVENT_TYPE_A_TOPIC";
        ...
        public async Task<PublishMsgResultsBE> PublishEvent(int eventId, string message)
        {
            ...
            var eventBody = new EventTypeABody()
            {
                id = eventId,
                name = message,
            ...
            // ====================================
            var eventPubResult = await KafkaUtilities.PublishMsgToTopic<EventKey, EventTypeABody>(TOPIC_NAME, eventKey, eventBody, _kafkaConfig);

            if (!eventPubResult.IsSuccess)
            {
                errTitle = $"Error in {nameof(EventTypeAPublisher)}";
